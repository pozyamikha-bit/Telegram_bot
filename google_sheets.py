...
# Лог рассылки купонов (админ-панель -> "📥 Импорт" -> "🎟 Купоны"). Каждая
# загрузка ДОБАВЛЯЕТ строки (история копится, как и у "Отчёт по продажам").
COUPONS_HEADER = ["Дата отправки", "Username", "Telegram ID", "Купон", "Номинал", "Статус"]
...
DEFAULT_TEXTS = {
    ...
    "ask_photo": (
        ...
        "После фото я попрошу вручную ввести номер УПД, а дату чека "
        "нужно будет выбрать в календаре."
    ),
    ...
    "ask_receipt_date": "Выберите дату чека в календаре ниже.",
    ...
    "accept_message": "Ваш купон № {coupon} по УПД № {upd_number} от {receipt_date}",
    "accept_no_coupon_message": "Ваш чек принят!",
    "coupon_import_message": "Ваш купон для OZON: {coupon}, с номиналом {nominal}",
    "instructions_admin": (
        "Инструкция по админ-панели пока не заполнена. Владелец бота может "
        "заполнить её через «✏️ Тексты» -> «Инструкция по админ-панели»."
    ),
    "instructions_receipt": (
        "Инструкция «Отправка чека пользователем» пока не заполнена. "
        "Владелец бота может заполнить её через «✏️ Тексты» -> "
        "«Инструкция по отправке чека пользователем»."
    ),
}
...
TEXT_LABELS = {
    ...
    "accept_message": (
        "Сообщение при принятии чека с купоном (внутри можно оставить "
        "{coupon}, {upd_number} и {receipt_date} — вместо них подставятся "
        "номер купона, номер УПД и дата чека)"
    ),
    "accept_no_coupon_message": "Сообщение при принятии чека БЕЗ купона",
    ...
    "instructions_admin": "Инструкция по админ-панели",
    "instructions_receipt": "Инструкция по отправке чека пользователем",
}
...
def normalize_upd(text) -> str:
    """Нормализует № УПД для сравнения: убирает пробелы по краям и
    приводит к одному регистру, чтобы, например, "abc123" и "ABC123"
    считались одним и тем же номером (регистр не должен влиять ни на
    проверку повтора, ни на сверку с отчётом по продажам)."""
    return (text or "").strip().upper()


def find_active_receipt_by_upd(upd_number: str):
    """Ищет уже существующий чек с таким же № УПД (без учёта регистра),
    который всё ещё "активен" — не удалён и не отклонён модератором.
    Используется, чтобы не дать подать один и тот же № УПД дважды (в том
    числе двум разным пользователям) — по правилам один чек даёт только
    один купон. Отклонённые чеки НЕ считаются занятыми — пользователю
    разрешено повторно подать чек с тем же № УПД, если модератор его
    отклонил (например, из-за плохого фото). Возвращает словарь чека (в
    формате get_receipts()) или None, если совпадений не нашлось."""
    upd_number = normalize_upd(upd_number)
    if not upd_number:
        return None
    for r in get_receipts():
        if r["deleted"] or r["status"] == STATUS_REJECTED:
            continue
        if normalize_upd(r["upd_number"]) == upd_number:
            return r
    return None
...
def append_coupons_log(rows):
    """rows: список [username, telegram_id, купон, номинал, статус] —
    записывает результат рассылки купонов ("📥 Импорт" -> "🎟 Купоны") в
    лист "Купоны". Дата отправки проставляется автоматически (одна и та
    же для всех строк одной рассылки). Как и "Отчёт по продажам" — новая
    рассылка ДОБАВЛЯЕТ строки, история копится, ничего не стирается."""
    if not rows:
        return
    ws = _get_worksheet(config.GOOGLE_SHEET_WORKSHEET_COUPONS, header=COUPONS_HEADER)
    sent_ts = _moscow_now_str()
    values = [[sent_ts] + list(row) for row in rows]
    ws.append_rows(values)
    _invalidate_records_cache(config.GOOGLE_SHEET_WORKSHEET_COUPONS)


def get_coupons_log():
    records = _get_records(config.GOOGLE_SHEET_WORKSHEET_COUPONS, header=COUPONS_HEADER)
    return [
        {
            "sent_date": str(rec.get("Дата отправки", "")),
            "username": str(rec.get("Username", "")),
            "telegram_id": str(rec.get("Telegram ID", "")),
            "coupon": str(rec.get("Купон", "")),
            "nominal": str(rec.get("Номинал", "")),
            "status": str(rec.get("Статус", "")),
        }
        for rec in records
    ]
